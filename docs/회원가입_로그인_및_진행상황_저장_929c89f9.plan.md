...
> **이 문서는 더 이상 유효하지 않습니다 (Obsolete).** Express + Prisma + PostgreSQL 기반의 자체 백엔드 인증 설계 초안이며, 이후 **Supabase Auth + Postgres + RLS** 기반의 서버리스 구조로 전환되었습니다.
> 최신 구조는 다음 문서를 참고하세요.
>
> - 전체 아키텍처: [docs/ARCHITECTURE.md](ARCHITECTURE.md)
> - API / Composables: [docs/API.md](API.md)
> - Supabase 마이그레이션: [supabase/migrations/001_user_progress.sql](../supabase/migrations/001_user_progress.sql)
